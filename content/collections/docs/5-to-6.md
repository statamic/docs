...
### Icon Fieldtype: `folder` option removed
**Affects apps or addons using Icon Fieldtype with the `folder` option.**

To simplify the Icon fieldtype, and avoid confusion about when to use the `directory` and `folder` options, the `folder` option has been removed.

If you use `folder` option, you should manually update Icon fields in your blueprints & fieldsets, and append the folder to the directory:

```yaml
 -
  handle: favourite_icon
  field:
    directory: resources/custom-icons # [tl! remove]
    folder: streamline-flex # [tl! remove]
    directory: resources/custom-icons/streamline-flex # [tl! add]
    type: icon
```

### Custom Set Icons: `$folder` parameter removed
**Affects apps or addons using the `Sets::setIconsDirectory()` method with the `$folder` parameter.**

Following changes to the Icon fieldtype, the `Sets::setIconsDirectory()` method no longer accepts a `$folder` parameter.

You should instead pass the folder as part of the first parameter:

```php
Sets::setIconsDirectory(directory: resource_path('custom-icons'), folder: 'streamline-flex') // [tl! remove]
Sets::setIconsDirectory(directory: resource_path('custom-icons/streamline-flex')) // [tl! add]
```
