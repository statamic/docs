...
### `webauthn` database table
**Affects apps storing users in the database.**
During the upgrade process, Statamic will attempt to publish a migration to create a `webauthn` table, used for storing passkeys. You will need to run `php artisan migrate`.

If the migration isn't published automatically, you may publish it manually via `php please auth:migration`. It will generate two migrations - you can safely delete the `update_users_table` one, then run `php artisan migrate`.
